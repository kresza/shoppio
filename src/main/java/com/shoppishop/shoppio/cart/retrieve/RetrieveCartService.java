...
public class RetrieveCartService {
  ...
  public BaseResponse getCartById(String cartId, PromoCodes promotion) {
    try {
      ...
      if (cartEntity == null) {
        throw new NoSuchElementException(String.format("Cart {%s} not found", cartId));
      }
      ...
      throw BusinessException.builder()
          .message(String.format("%s%s", ErrorEnum.NOT_FOUND_CART_ERROR.getMessage(), cartId))
          .code(ErrorEnum.NOT_FOUND_CART_ERROR.getCode())
          .build();
    } catch (Exception e) {
      throw BusinessException.builder()
